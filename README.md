...
The frozen decision is `IMMUNE_CISEQTL_FULL24 = FROZEN_NO_GO`. REEP3 remains a discovery-only result and is not counted as a replicated locus. The next bounded stage audits public cis-pQTL bytes and tests donor-level kidney localization for the validated ZMIZ1 anchor.
