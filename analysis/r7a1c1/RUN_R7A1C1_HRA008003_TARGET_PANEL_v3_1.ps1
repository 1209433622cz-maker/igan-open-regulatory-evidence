...
if($PreflightOnly) {
  ...
  $wslSchemaScript = Convert-ToWslPath (Join-Path $Code '08_check_HRA008003_bam_schema.py')
...
foreach($item in $manifest) {
  ...
  if(Test-Path -LiteralPath $summaryPath) {
    ...
    $prior = @($receipts | Where-Object {$_.run -eq $run})
    $summarySha = [string]$existing.bam_sha256
    $summaryBytes = [int64]$existing.bam_bytes
    $validSummary = (
      $existing.schema_version -eq 'CMM_R7A1C1_TARGET_PANEL_2.0' -and
      $existing.technical_QC -eq 'PASS' -and
      $existing.run -eq $run -and
      $summaryBytes -eq [int64]$item.expected_bytes -and
      $summarySha -match '^[0-9a-f]{64}$'
    )
    $validReceipt = (
      $prior.Count -eq 1 -and
      $prior[0].status -match '^PASS_TARGET_PANEL_V2' -and
      [int64]$prior[0].bytes -eq [int64]$item.expected_bytes -and
      $prior[0].official_md5 -eq $item.official_md5 -and
      $prior[0].observed_md5 -eq $item.official_md5 -and
      $prior[0].sha256 -eq $summarySha
    )
    if($validSummary -and $validReceipt) {
