...
foreach($required in @(
  ...
  (Join-Path $Code '08_check_HRA008003_bam_schema_v1_2.py'),
  (Join-Path $Code '11_validate_HRA008003_resume_state.py')
)) {
...
if($PreflightOnly) {
  $wslTargetScript = Convert-ToWslPath (Join-Path $Code '02_hra_bam_target_panel_v2.py')
  $wslSchemaScript = Convert-ToWslPath (Join-Path $Code '08_check_HRA008003_bam_schema_v1_2.py')
  & wsl.exe -d Ubuntu-22.04 -- bash -lc "test -f '$wslTargetScript' && test -f '$wslSchemaScript'"
  if($LASTEXITCODE -ne 0) { throw 'WSL path conversion or script visibility preflight failed' }
  [pscustomobject]@{
    status='PASS_R7A1C1B1R_PREFLIGHT_ONLY'
...
foreach($item in $manifest) {
  ...
  if(Test-Path -LiteralPath $summaryPath) {
    if(!(Test-Path -LiteralPath $receiptPath)) {
      throw "Existing summary has no persisted receipt table: $summaryPath"
    }
    $resumeOut = Join-Path $Audit "$run.resume_validation_v3_2.json"
    & python (Join-Path $Code '11_validate_HRA008003_resume_state.py') `
    ...
    if($LASTEXITCODE -eq 0) {
      Write-Host "VERIFIED_V2_SUMMARY_AND_RECEIPT_EXIST $run"
      continue
    }
    throw "Existing summary lacks a matching validated byte/MD5/SHA receipt: $summaryPath"
  }
  ...
  $schemaScriptWsl = Convert-ToWslPath (Join-Path $Code '08_check_HRA008003_bam_schema_v1_2.py')
  ...
  $receipts += [pscustomobject]@{
    ...
    status='PASS_TARGET_PANEL_V2_VIA_RUNNER_V3_2'; bam_deleted=$false
  }
  $receipts | Sort-Object run | Export-Csv -LiteralPath $receiptPath -Delimiter "`t" -NoTypeInformation -Encoding utf8

  # The persisted receipt must independently validate before deletion is allowed.
  $resumeOut = Join-Path $Audit "$run.resume_validation_v3_2.json"
  & python (Join-Path $Code '11_validate_HRA008003_resume_state.py') `
    --summary $summaryPath --receipts $receiptPath --run $run `
    --expected-bytes $item.expected_bytes --official-md5 $item.official_md5 --out $resumeOut
  if($LASTEXITCODE -ne 0) {
    throw "New summary/receipt identity validation failed; BAM retained: $run"
  }
