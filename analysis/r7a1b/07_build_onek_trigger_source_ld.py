...
ap=argparse.ArgumentParser();ap.add_argument("--gene",required=True);ap.add_argument("--cell",required=True)
a=ap.parse_args()
...
Q=ROOT/"3_results/03_qtl/R7A1B/R7A1B_OneK_frozen_QTL.tsv.gz"
OUT=ROOT/f"3_results/04_integration/R7A1B/sourceLD/{a.gene}_{a.cell}";OUT.mkdir(parents=True,exist_ok=True)
fam=pd.read_csv(FAM,sep=r"\s+",header=None,dtype=str);ids=fam.iloc[:,1].tolist();idx={x:i for i,x in enumerate(ids)}
keep_ids=[x for x in S.read_text(encoding="utf-8").splitlines() if x];keep=np.array([idx[x] for x in keep_ids])
bim=pd.read_csv(BIM,sep=r"\s+",header=None,names=["chr","variant_id","cm","pos","A1","A2"],dtype=str);bim["gi"]=np.arange(len(bim))
q=pd.read_csv(Q,sep="\t");q=q[(q.gene==a.gene)&(q.cell_type==a.cell)].drop_duplicates("variant_id")
v=q.merge(bim[["variant_id","gi","A1","A2"]],on="variant_id",how="inner",validate="one_to_one")
if len(v)!=len(q):raise RuntimeError("QTL/BIM mismatch")
bps=(len(ids)+3)//4;G=np.empty((len(keep),len(v)),dtype=np.float32)
with BED.open("rb") as f:
    if f.read(3).hex()!="6c1b01":raise RuntimeError("BED magic")
    for j,gi in enumerate(v.gi.astype(int)):
        ...
        G[:,j]=dos
sd=G.std(0,ddof=1);good=np.isfinite(sd)&(sd>0);G=G[:,good];v=v.loc[good].reset_index(drop=True)
G=(G-G.mean(0))/G.std(0,ddof=1);R=(G.T@G)/(G.shape[0]-1);R=(R+R.T)/2;np.fill_diagonal(R,1)
np.save(OUT/"standardized_A1_dosage.npy",G.astype(np.float32));np.save(OUT/"raw_A1_correlation.npy",R.astype(np.float32))
(OUT/"active_donors.txt").write_text("\n".join(keep_ids)+"\n",encoding="utf-8")
v.insert(0,"LD_order_zero_based",np.arange(len(v)));v.to_csv(OUT/"LD_variants.tsv",sep="\t",index=False)
qc={"gene":a.gene,"cell":a.cell,"n_samples":len(keep),"n_variants":len(v),"finite":bool(np.isfinite(R).all()),
    "max_asymmetry":float(np.max(abs(R-R.T))),"max_diag_dev":float(np.max(abs(np.diag(R)-1))),
    "min_eigenvalue":float(np.linalg.eigvalsh(R.astype(float)).min())}
